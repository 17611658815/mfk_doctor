...
Page({
    ...
    startRecording() {
        var that = this
        if (that.data.record_txt.length<100) {
            app.alert('文字内容不可少于100字哦~')
        } else {
            //录音开始
            ...
            // 记录录音时长
            that.data.timers = setInterval(function () {
                num++;
                timeOrder--;
                if (num > 29) {
                    that.setData({
                ...
                var time = num;
                var second = time % 60;
                console.log(num, that.data.timers)
    ...
    recordingHint() {
        wx.navigateTo({
            url: '/pages/recordingHint/recordingHint?type=2',
        })
    },
    textaerChange(e){
        console.log(e)
