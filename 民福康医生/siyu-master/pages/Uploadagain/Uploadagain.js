...
Page({
  ...
  onShow() {
    var that = this;
      var signImg = app.globalData.protocol || "";
    wx.getSystemInfo({
      success: (res) => {
        that.setData({
  ...
  checkauth: function () {
    ...
    getDocterMsg(phone) {
        ...
        app.net.$Api.getDocterMsg(params).then((res) => {
            console.log(res)
            let identity = res.data.msg.identity.split(',')
            ...
            that.data.userInfoAvatar = res.data.msg.avatar,
             that.data.docInfo.xyImg = res.data.msg.cooperation
                // 三证回显
                that.data.imgs[0] = res.data.msg.avatar,
                that.data.imgs2 = identity,
    ...
    if (that.data.docInfo.userPic == '') {
      ...
      wx.request({
        ...
        data: {
          ...
          cooperation: that.data.docInfo.xyImg,
